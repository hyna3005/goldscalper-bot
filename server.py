...
async def receive_alert(secret: str, request: Request):
    ...
    # Xu ly DONG alert tu V45 (SL/TP/ST tren chart)
    # Format: "DONG BUY SL @ 4457.73 | 14:35 (VN)"
    #         "DONG BUY ST @ 4462.00 | 14:35 (VN)"
    if raw.startswith("DONG ") and ("SL @" in raw or "ST @" in raw or "TP @" in raw):
        import re as re2
        dong_type = "SL" if "SL @" in raw else "TP" if "TP @" in raw else "ST"
        dong_dir  = "BUY" if "BUY" in raw else "SELL"
        price_m   = re2.search(r"@\s*([\d.]+)", raw)
        dong_price = float(price_m.group(1)) if price_m else 0

        # Gui lenh dong den EA
        EA_COMMAND["cmd"] = "close"

        msg = ("CHART " + dong_type + " | " + dong_dir + "\n"
               "V45 bao dong @ " + str(round(dong_price, 2)) + "\n"
               "Da gui lenh CLOSE den EA")
        print(msg)
        await tg(ADMIN_ID, msg)

        # Forward Telegram
        if GROUP_CHAT_ID:
            await tg(GROUP_CHAT_ID, f"DONG {dong_dir} {dong_type} @ {dong_price:.2f}")
        for m in get_active_members():
            await tg(m["user_id"], f"DONG {dong_dir} {dong_type} @ {dong_price:.2f}")
            await asyncio.sleep(0.05)

        return JSONResponse({"status": "close_sent", "type": dong_type, "price": dong_price})
